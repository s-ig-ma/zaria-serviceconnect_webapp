...
export function ResidentMessagesPage({ session, onNavigate, onLogout }) {
  ...
  return (
    <Layout
      session={session}
      title="Messages"
      subtitle="Chat directly with admin about your complaint. Provider messages do not appear here."
    ...
    >
      <section className="messages-shell">
          ...
          <div className="stack-list">
            {complaints.map((complaint) => (
              ...
              >
                ...
                <span>
                  Against: {complaint.provider?.user?.name || "Provider"}
                </span>
              </button>
            ))}
          </div>
        ...
        <div className="panel message-stage">
          ...
          {selectedComplaint ? (
            <>
              <div className="message-stage-header resident">
                <div>
                  <span className="eyebrow">Resident Inbox</span>
                  <h2>Complaint #{selectedComplaint.id}</h2>
                  <p>
                    Admin support chat about {selectedComplaint.provider?.user?.name || "this provider"}
                  </p>
                </div>
                <div className="message-stage-badge resident">Resident</div>
              </div>
                ...
                {messages.map((item) => (
                  <article key={item.id} className={getMessageRoleClass(item.sender.role)}>
                    <div className="message-bubble-head">
                      ...
                      <span>{item.sender.role === "admin" ? "admin" : "resident"}</span>
                    </div>
                    <p>{item.content}</p>
                  </article>
                ))}
              ...
              <div className="message-composer resident">
